...
bool containDuplicateLower(vector<string> path);
...
vector<vector<string>> bfs(const vector<string> &curPath, std::unordered_map<string, std::list<string>> & g){
    ...
    for(string s : linkedNode){
        if(s == END){
            ...
            //New Rule: single small node can be visited twice, remaining small node only once.
            int visited = std::count(curPath.begin(), curPath.end(), s);

            if(visited == 0 || !containDuplicateLower(curPath)){
                vector<string> newPath(curPath.begin(), curPath.end());
                newPath.push_back(s);
                vector<vector<string>> rest = bfs(newPath, g);
                for(int i=0; i<rest.size(); i++){
...
//vector<string> path contains duplicate of lowercase letter node
// true: already contains duplicate of lowercase letter node
bool containDuplicateLower(vector<string> path){
    set<string> uniqueSet;
    for(int i=0; i<path.size(); i++){
        if( isLower(path[i]) ){
            set<string>::iterator it = uniqueSet.find(path[i]);
            if(it == uniqueSet.end()){
                uniqueSet.insert(path[i]);
            }else{
                return true;
            }
        }
